...
**実データ検証——2026年7月を実際の価格で再現。** 2008年のレシピを**凍結**したまま（重み・閾値・持続ルール不変、資金ストレス→過密延伸の一箇所のみ開示済み適配）、メモリ・バスケット（SK hynix・サムスン・Micron・WD・Seagate・SanDisk、データ同梱・キー不要）を今月の崩落までウォークフォワード再生：`python demo/hindcast_memory_2026.py`。結果——3年間誤警報ゼロ、全サンプル初の警報は**2025年10月22日**（AIメモリ・マニアそのもの）、**2008年以来最悪の月の232営業日前**。そして誠実なもう半分：警報後バスケットはさらに**+446%**上昇してから崩れた。レジーム不安定の検出は暴落のタイミングではない——*いつ*崩れるかは**誰がくさびを支え、いつ撤退するか**（κ）が決める。それがまさに上の合成デモの信号の後半であり、実験E7の中身だ。「予測した」という主張はしない——事後計算のヒンドキャストであり、図にもそう明記してある。

[![ネットワークはどう崩落を推断するか——agent即ニューロン・主結合・メカニズム](../../figures/agent_game_inference.png)](../../scripts/make_agent_game_figure.py)

*予測の作られ方を一枚で——**(a)** 全結合agentネットワーク（ハイライトの5本が主成分分析のような**主結合**）；**(b)** その負荷量；**(c)** それらが生成する崩落メカニズム。agentゲームの分析モデリング、ニューラルネットワークの新応用、その階層と構造——一枚の図で。*

**なぜ可能なのか？市場に存在するあらゆる既存手法との3つの決別——それがこのMVPの核心であり魂だ：**

**① 因果帰属——深層学習・強化学習・ファクターマイニングが構造的に解けない問題。** 価格データで訓練されたモデルは意思決定の*残渣*から学ぶが、残渣は何も説明しない。我々がモデル化するのは**意思決定者そのもの**：現実世界のデータで市場の実プレイヤー——機関・規制当局・個人投資家コホート——を、目的関数・制約・情報集合を持つagentとして全結合ネットワーク上でゲームさせる。今日は多体・多次元の階層的平均場系への簡約形（第1段階）、非簡約の完全形——**各agentがそれ自体ニューラルネットワークで共同訓練**——は[NNGS](../PHASE2_NEURAL_GAME.md)に仕様化済みで、ハードウェアを待つ（H200を200基持つラボがあれば届く——門は開いている）。どちらの形でも出力は同じ分解を持つ：どの主体、どの制約、どの結合か。**因果帰属はここではネイティブであり、後付けではない。**

**② 情報格差こそ予測のエンジン。** 価格が動くのは、異なる情報を持つプレイヤーが異なるタイミングで動くからだ。本フレームワークは市場の階層化された情報構造を明示的にモデル化する：各主体タイプは自分のフィルトレーションしか見えず、モデルは「それだけが見えている状態での合理的な一手」を型ごとに推定する——価格変動は、テープに集約される前、**プレイヤー間の情報ギャップの中**で先取りされる。2026年7月はこのメカニズムの実例：サプライチェーン調査が機関に届いたのは、ヘッドラインが個人投資家に届く数ヶ月前だった。

**③ これは「金融エージェント」プロダクトの亜種ではない。** 現在のLLM金融エージェントは研究部門のロールプレイだ——「マネージャー」「アナリスト」「リスク担当」が同じ公開ヘッドラインを囲んで会議する。ロールプレイは情報を製造しない：個人投資家は依然として洗浄済み・高SN比のデータを受け取れず、同じプロンプトを走らせるユーザーが増えるほど、残っていたエッジはあらゆるファクターと同じ速度で減衰する。我々のagentは配役ではなく**市場の実プレイヤー型の構造モデル**であり、ユーザーが受け取るのは高SN比の成果物そのもの——ノイズ除去済み均衡価格だ。
