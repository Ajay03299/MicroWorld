...
[![네트워크는 어떻게 폭락을 추론하는가 — agent 즉 뉴런·주결합·메커니즘](../../figures/agent_game_inference.png)](../../scripts/make_agent_game_figure.py)

*예측이 만들어지는 방식을 한 장으로 — **(a)** 완전연결 agent 네트워크(하이라이트된 5개 엣지가 주성분분석 같은 **주결합**); **(b)** 그 로딩; **(c)** 그것들이 생성하는 폭락 메커니즘. agent 게임의 분석 모델링, 신경망의 새로운 응용, 신경망의 계층과 구조 — 한 장의 그림.*
