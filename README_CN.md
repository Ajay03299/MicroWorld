...
**真实数据检验——2026年7月，在真实价格上重放。** 我们把 2008 年那套配方**冻结**——权重、阈值、持续规则原封不动，只做一处公开声明的适配（资金压力→拥挤延伸）——然后在存储篮子（SK 海力士、三星、美光、西部数据、希捷、SanDisk；日线收盘价已内置于仓库，无需密钥）上逐日 walk-forward 重放，穿过本月的崩盘：`python demo/hindcast_memory_2026.py`。
