...
> *雷曼兄弟申请破产前的最后一个星期五，VIX 收于 25.7——偏高，但寻常。而在十三个月之前，本模型的危机指标就已经离开了稳定域。*

[![2008 年危机回放——仅用公开数据的 walk-forward 重放](figures/hindcast_2008.png)](demo/hindcast_2008.py)

**仅用公开数据对 2005–2009 的 walk-forward 回放**——Ken French 日频因子、FRED 资金利差；数据已内置于仓库，零 API 密钥，一条命令：`python demo/hindcast_2008.py`。每个日期的统计量只使用当天早晨可得的信息：

- 2005–2006 全程**零误报**。
- **2007年8月10日**——BNP Paribas 冻结三只基金后的第一个交易日——Λₜ 首次收于危机阈值之上。
- **2007年8月16日**——Λₜ 进入*持续*危机域：**比雷曼早 272 个交易日**。从该信号到 2009 年 3 月的底部，市场下跌 **−49%**。
- **2008年9月12日**，雷曼前的最后一个收盘日：资金压力再度加速（TED 五个交易日内 1.13 → 1.36），Λₜ 重回均衡之上。

*这是一次 hindcast——无前视偏差的回测，而非实时预测。2008 年诚实的教训不是"崩盘可以精确到某一天被预言"，而是：对一个盯着状态空间几何——资金、相关性、波动率——的模型来说，**稳定性的丧失提前整整一年就清晰可见**，而基于价格的仪表盘当时读作"寻常"。Λₜ 度量的是与均衡盆地的距离（定理8.2）；2008 就是"盆地逃逸"在真实数据中的样子。*

**那么，这样一个能看见危机逼近的世界模型，到底是怎么构建的？这个问题，就是整个仓库本身。** ↓
...
> 一切始于斯坦福一间地下室的深夜，三个学生放不下同一个问题：
>
> **为什么每一个交易模型，都在上线的那一刻开始死亡？**
>
> 答案不是更好的数据，也不是更大的网络。市场不是数据集——市场是博弈。价格来自五万家机构、五亿散户和三十家央行的策略性互动——它们同样在对每一个胆敢预测它们的模型做出反应。你预测模式，模式就消失。唯一剩下可以建模的，是玩家本身。
>
> 二十一天后，就有了这个仓库：**第一个为金融市场而生的开源世界模型架构**——建模玩家而非模式，让预测被设计为能在自身部署后存活。数学已证明，代码在一台笔记本上就能跑，通往实盘系统的路线图精确到每一个数据文件。
>
> 这是 Alpha Flow 的第一件公开作品。赌注已经下了——当着所有人的面。接下来会发生什么，取决于谁加入进来。
...
![四层耦合市场网络——约5000个主体，一个嵌套平均场博弈](figures/four_level_network.png)
