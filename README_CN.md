...
> *把行为噪声从价格中剥离，剩下的就是均衡——当每一个玩家都打出自己的策略后，这个资产真正值多少钱。**我们预测的就是这条轨迹。** 不是明天的跳动价位：是去噪价格，为以月计而非以毫秒计持有的人准备。*

[![去噪均衡价格——MVP 演示](figures/denoised_price_demo.png)](demo/denoised_price_2026.py)

**一条命令，零 API 密钥：** `python demo/denoised_price_2026.py`——以 2026 年 7 月存储板块崩盘（费城半导体指数月跌 19%，2008 年以来最差单月）为情境原型的**合成数据**概念演示：

- **蓝线是产品**——世界模型求解出的均衡轨迹 **P^eq**。**黑线是市场**——均衡*加上*一个行为楔子：机构越拥挤、被 LLM 同质化的散户越追高，楔子越膨胀。
- **信号**——偏离度 D_t = P_t/P^eq_t − 1 突破阈值*且机构平均场正在撤出*——在崩盘加速前 **11 个交易日（约两周）** 触发。散户在低点投降，机构在同一低点接盘，价格重新贴回它的去噪轨迹。
- **服务对象：** 中长期持有者（To-C，散户）。刻意**不**服务日内与高频交易者——在那些频率上行为噪声占主导，且定理 1（对偶 Cramér-Rao 界）证明再多数据也无法把它建模掉。而在周到月的尺度上，均衡分量占主导——这恰恰是本模型为之设计的战场。*（研究信号，非投资建议。真实数据版本沿用 2008 回测同一套无前视规则，即实验 [E7](RESOURCES.md)。）*

**真实数据检验——2026年7月，在真实价格上重放。** 我们把 2008 年那套配方**冻结**——权重、阈值、持续规则原封不动，只做一处公开声明的适配（资金压力→拥挤延伸）——然后在存储篮子（SK 海力士、三星、美光、西部数据、希捷、SanDisk；日线收盘价已内置于仓库，无需密钥）上逐日 walk-forward 重放，穿过本月的崩盘：`python demo/hindcast_memory_2026.py`（图见[下方战绩存根](#同一引擎两条产品线)）。

- **三年零误报**——穿过 2022 年存储熊市和 2023–25 年复苏，Λₜ 一次都没有越过阈值。
- **全样本第一次报警：2025 年 10 月 22 日**——恰恰是这场 AI 存储狂热本身。板块离开稳定域的时刻，**比 2026 年 7 月交出 2008 年以来最差单月早 232 个交易日**（内置数据中 SK 海力士自峰值 −55%）。
- **以及诚实的另一半：** 报警之后，篮子又上涨了 **+446%** 才断裂。盆地逃逸检测不等于崩盘择时——一个不稳定的拥挤机制可以继续膨胀数月。*何时*断裂，取决于**谁在撑住楔子、他们何时撤出**——机构平均场 κ。这个 κ 转向条件，正是上方合成演示中 MVP 信号的第二半，而用真实 13F/COT 持仓数据重建它，就是实验 [E7](RESOURCES.md)。

我们不作任何"我们预测到了"的声明——这是事后计算的 hindcast，图上白纸黑字写着。它证明的是同一个世界模型内部的分工：**Λₜ 告诉你机制不稳定；带 κ 转向条件的偏离度 D_t 告诉你不稳定何时兑现。** 合成演示展示了第二半如何工作；E7 让它成真。

[![网络如何推断崩盘——agent即神经元、主耦合、机制](figures/agent_game_inference.png)](scripts/make_agent_game_figure.py)

*预测是怎么做出来的，一张图讲完——**(a)** 全连接 agent 网络：每个神经元是一个 agent，五条高亮的边是**主耦合**——像主成分分析一样，承载这一轮行情的少数几条关键相互作用；**(b)** 它们的载荷；**(c)** 这些耦合生成的崩盘机制：信息先到达知情者（w₁），机构去拥挤（w₂），动量把散户拉进来（w₃、w₄），做市商库存决定跳空风险（w₅），楔子最终塌缩回均衡。agent 博弈的分析建模、神经网络的新应用、神经网络的层次与结构——一张图。*

**这为什么可能？与市面上一切现有做法的三次决裂——它们是这个 MVP 的核心与灵魂：**

**① 因果归因——深度学习、强化学习、因子挖掘在结构上无法解决的问题。** 在价格数据上训练的模型，学的是决策留下的*残渣*，而残渣解释不了任何东西——这就是每个回测出身的信号都死得不明不白的原因。我们建模的是**做决策的人**：用现实世界的真实数据刻画市场的真实玩家类型——机构、监管者、散户群体——把他们作为带目标函数、约束和信息集的 agent，放进一张全连接网络中博弈。今天我们求解的是这个博弈的简化形态——一个多体、多维度的层级平均场系统（第一阶段）；未简化的完全形态——**每个 agent 本身是一个神经网络、共同训练**——已在 [NNGS](docs/PHASE2_NEURAL_GAME.md) 中写成规格，等待硬件（一个拥有两百张 H200 的实验室就能补上这一步——[大门敞开](#合作与联系)）。无论哪种形态，每个输出都以同一种方式分解：哪些主体、哪条约束、哪个耦合。**因果归因在这里是原生的，不是事后补的。**

**② 信息差是预测的引擎。** 价格之所以变动，是因为掌握不同信息的玩家在不同时刻行动。本框架显式建模市场的分层信息结构（[`state/information.py`](state/information.py)）：每类主体只看得见自己的信息流，模型据此估计每类主体*在它所能看见的范围内*的理性决策必然是什么——于是价格变化在它真正诞生的地方——**玩家之间的信息差里**——就被预判，而不是等它汇聚成盘面之后。2026 年 7 月就是这个机制的野外实例：产业链调研到达机构比头条新闻到达散户早了几个月，这两个信息集之间的楔子*就是*崩盘的力学本身。

**③ 这不是又一个"金融 agent"产品。** 当前这波 LLM 金融 agent 在角色扮演一个研究部门——一个"基金经理"、一个"分析师"、一个"风控专家"，围着同样的公开头条开会。角色扮演制造不出信息：散户用户依然拿不到清洗过的、高信噪比的数据；而当成千上万的用户跑着相同的 agent 提示词处理相同的输入，那点残存的优势会以每个因子衰减的同样速度被挤干。我们的 agent 不是人设——它们是**市场真实玩家类型的结构模型**，用户拿到的是那个高信噪比的成果本身：去噪均衡价格。[我们没有在数据里找到结构——我们建模了生成数据的结构 ↓](#第一性原理之赌因果而非相关)

*机构线是同一引擎的反向运行——包括那次危机指标**比雷曼早 272 个交易日**离开稳定域、仅用公开数据且无前视偏差的 2008 回测：见[同一引擎，两条产品线](#同一引擎两条产品线)。*
...
6. [市场作为分层博弈——真实管道与博弈投影](#市场作为分层博弈真实管道与博弈投影)
...
语言模型方法在文本理解上令人印象深刻，但缺乏市场机制的根基。它们无法满足基本的无套利约束，没有均衡理论，其输出把语言连贯性和金融有效性混为一谈。而它的产品化形态——角色扮演式 agent 团队，一个"基金经理"、一个"分析师"、一个"风控专家"围着同样的公开头条开会——继承了以上全部三个问题，还新增了第四个：当相同的 agent 提示词扩散到成千上万的用户手中，它们的结论会像任何已发表的因子一样拥挤并衰减（见上方 MVP 的决裂③）。
...
![市场作为分层神经网络——资金流0-6层、自营/高频平行层与两个外圈](figures/four_level_network.png)

上图不是比喻。市场**就是**一张分层神经网络——只是神经元是主体：它们不放电，它们**做决策**。七列就是资金流的七层；∥ 簇是用自有资金在 L4-L5 之间做流动性的自营/高频平行层；两个虚线外圈从不经手资金——信息环引导资金流向，规则环施加约束（红色虚线框就是一个共享约束模块：同一监管者，同一套权重）。每条边是策略耦合而非被动权重，底部蓝色回流箭头闭合整个环路——反身性。第一阶段以层级平均场博弈求解这张图的四层博弈投影；第二阶段（[NNGS](docs/PHASE2_NEURAL_GAME.md)）直接训练完整的图——每个神经元一个小神经网络。这也是这张图长成这个样子的原因。

中国有句古话：**个人由环境造就**。我们的框架将这一洞察精确化。各层并非孤立：每个主体同时是上层的产物，也是上层的贡献者。环境不是外部噪声——它是所有其他主体策略的总和。

### 真实管道：资金流七层 + 一个平行层 + 两个外圈

市场角色的分层依据是**资金流动的方向**——而不是四个方便叙述的档位。完整分类见 **[docs/MARKET_LAYERS.md](docs/MARKET_LAYERS.md)**；骨架如下：

| 层 | 谁 | 决定什么 |
|---|---|---|
| **0** · 原始盈余部门 | 散户、高净值、企业财资、财政盈余 | 储蓄从哪里进入 |
| **1** · 机构化出资人 | 养老金（CalPERS、GPIF）、保险、主权基金、捐赠基金 | 负债驱动的配置授权 |
| **2** · 配置与导管层 | 投资顾问/OCIO、私人银行、基金平台、银行存贷体系 | 钱给谁管（不直接下单） |
| **3** · 资产管理人 | 公募/指数/ETF、对冲基金、私募市场 | 持仓——alpha 竞争层 |
| **4** · 卖方与交易中介 | 投行、prime broker、交易商 | 通道、杠杆、价差 |
| **5** · 市场基础设施 | 交易所、CCP/CSD、托管行、支付网络 | 钱与证券实际交割之处 |
| **6** · 资金需求方 | 企业、财政部、证券化SPV | 对资本的需求 |
| **∥** · 平行层 | 自营做市与高频（Jane Street、Citadel Securities、Virtu…） | 只用自有资金，在4-5层之间赚流动性 |
| **◎1** · 信息外圈 | 指数公司、评级机构、数据商、媒体 | 不碰钱，却决定钱往哪流 |
| **◎2** · 规则外圈 | 央行、SEC/CFTC/Fed、FDIC、BIS | 约束与最后支持 |

> 储蓄（0）→ 机构化（1）→ 配置（2）→ 持仓（3）→ 通道（4）→ 交割（5）→ 发行人（6）→ 实体经济——报酬沿原路回流。

模型直接利用的两个结构性事实：**外圈2就是约束架构**——每个主体类型共享的监管投影，即上图中画出的权重共享；**外圈1就是信息架构**——分层信息流与引导资金的算子（一次指数纳入就是作用在 μ 上的 Mode-I 事件）。层次按功能而非公司划分：JPMorgan 同时活在第2、3、4、5层；散户身兼第0与第3层；央行身兼第1层与◎2。

### 数学所解的投影

第一阶段并不正面强攻这张十种角色的图——它求解的是这张图的**博弈投影**：四个嵌套的策略聚合层级（定理7.4的层级结构），把管道粗粒化为：*博弈L0* ≈ 整条链上的跨市场总资金流；*博弈L1* ≈ 从第1-4层与平行层中提取的策略类别；*博弈L2* ≈ 机构个体；*博弈L3* ≈ 交易台与个人。**第二阶段（[NNGS](docs/PHASE2_NEURAL_GAME.md)）抛弃投影，直接吞下完整的分层图。** 下面的四个博弈层级，就是那个投影：
...
**这就是本 README 顶部的 MVP。** 定理1说明：行为噪声 ν_η 是任何人都无法通过交易战胜的——散户尤其如此。一个长期投资者真正需要的，是噪声*之下*的那个分量：**均衡轨迹 P^eq**——当每一个主体都打出理性策略后，这个资产值多少钱——以及偏离度 **D_t = P_t/P^eq_t − 1**。它只回答一个问题：*你买入的是价值，还是拥挤？* 概念演示：[`demo/denoised_price_2026.py`](demo/denoised_price_2026.py)（合成数据，已明确标注）。真实数据验证：实验 **[E7](RESOURCES.md)**。
...
*同一套配方冻结后重放 2026 年 7 月（下图）：三年样本的第一次报警落在 **2025 年 10 月 22 日**——比投降早 232 个交易日——中间还隔着那诚实的 +446% 上涨。完整解读见[MVP 的真实数据检验](#mvp预测市场去噪之后的价格)。*

[![存储板块 2026 回放——冻结的 2008 配方作用于真实数据](figures/hindcast_memory_2026.png)](demo/hindcast_memory_2026.py)
...
│   ├── hindcast_2008.py       #   2008滚动前向回测
│   ├── hindcast_memory_2026.py#   存储2026：冻结2008配方，真实数据
...
# 2c · 存储板块2026回放——冻结的2008配方，真实内置数据
