...
上图不是比喻。市场**就是**一张四层神经网络——只是神经元是主体：它们不放电，它们**做决策**。每条边是策略耦合而非被动权重；每个虚线分组共享其监管者的约束模块（现实中的监管，恰好就是权重共享）；整张图由价格反馈环闭合——反身性。第一阶段以层级平均场博弈求解这张网络；第二阶段（[NNGS](docs/PHASE2_NEURAL_GAME.md)）直接训练它——每个神经元一个小神经网络。这也是这张图长成这个样子的原因。
